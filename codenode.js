...
files.push({
    ...
    content: [
        ...
        ...paired.flatMap(({ agent, instanceId }) => {
            ...
            return [
                `if [ -d "$DASH_SRC/${slug}" ]; then`,
                `  cp -r "$DASH_SRC/${slug}" "$DASH_DEST/${instanceId}"`,
                ...
                `  fi`,
                `  echo "[ClawMode] Dashboard: ${slug} -> ${instanceId}"`,
                `fi`,
            ];
        }),
        '',
        // Inject active agent instanceIds into hub page
        'ACTIVE_SLUGS="' + paired.map(p => p.instanceId).join(',') + '"',
        'if [ -f "$DASH_DEST/hub/index.html" ]; then',
        '  sed -i "s|__CLAWMODE_AGENTS_PLACEHOLDER__|$ACTIVE_SLUGS|" "$DASH_DEST/hub/index.html" 2>/dev/null || true',
        '  echo "[ClawMode] Hub configured with agents: $ACTIVE_SLUGS"',
        'fi',
        '',
        ...
        'cd "$DASH_DEST"',
        'su -s /bin/sh node -c "python3 -m http.server 3333 --bind 0.0.0.0 > /dev/null 2>&1 &" || true',
        ...
        '  var url = location.protocol + "//" + host + "/hub/";',
