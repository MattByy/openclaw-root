...
files.push({
    ...
    content: [
        ...
        ...paired.flatMap(({ agent, instanceId }) => {
            const slug = agent.slug || instanceId || 'unknown';
            return [
                `RAW_ID="${instanceId}"`,
                `DASH_NAME=$(resolve_dash "${slug}")`,
                `[ -z "$DASH_NAME" ] && DASH_NAME=$(resolve_dash "$RAW_ID")`,
                `if [ -n "$DASH_NAME" ] && [ -d "$DASH_SRC/$DASH_NAME" ]; then`,
                `  # Sanitize: if RAW_ID is empty or starts with '-', prefix with DASH_NAME`,
                `  case "$RAW_ID" in`,
                `    ""|-*) INST_ID="$DASH_NAME$RAW_ID" ;;`,
                `    *)    INST_ID="$RAW_ID" ;;`,
                `  esac`,
                `  rm -rf "$DASH_DEST/$INST_ID"`,
                `  cp -r "$DASH_SRC/$DASH_NAME" "$DASH_DEST/$INST_ID"`,
                `  D="$DASH_DEST/$INST_ID/index.html"`,
                `  if [ -f "$D" ]; then`,
                `    sed -i "s#window.__GATEWAY_TOKEN__ = window.__GATEWAY_TOKEN__ || null#window.__GATEWAY_TOKEN__ = '$OPENCLAW_GATEWAY_TOKEN'#" "$D"`,
                `    sed -i "s#window.__AGENT_ID__ = window.__AGENT_ID__ || null#window.__AGENT_ID__ = '$INST_ID'#" "$D"`,
                `  fi`,
                `  [ -n "$HUB_MAP" ] && HUB_MAP="$HUB_MAP,"`,
                `  HUB_MAP="$HUB_MAP$INST_ID:$DASH_NAME"`,
                `  echo "[ClawMode] Dashboard: ${slug} -> $DASH_NAME -> $INST_ID"`,
                `else`,
